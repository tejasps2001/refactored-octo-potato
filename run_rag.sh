...
# -------------------------------------------------------------------------
# 4. Surgical Dependency Provisioning
# -------------------------------------------------------------------------
# Only provisions environments for components that are actually actively running
...
# 5. Execution Handoff
# -------------------------------------------------------------------------
echo "⚡ Starting selected developmental components..."

# A. Start Emotion State Broadcaster API
...
# B. Start MediaPipe Camera Tracking Loop
...
# D. Start Streamlit User Interface
if [ "$RUN_FRONTEND" = true ]; then
    cd "$FRONTEND_DIR"
    source .venv/bin/activate
